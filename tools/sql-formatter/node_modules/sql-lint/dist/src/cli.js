...
(async () => {
    ...
    const configuration = (program.config)
        ? (0, config_1.getConfiguration)(program.config)
        : (0, config_1.findConfiguration)();
    ...
    if (program.fix) {
        ...
        if (typeof program.fix === "boolean") {
            query = (0, reader_1.getQueryFromLine)(fs.readFileSync(0).toString());
        }
        else {
            query = (0, reader_1.getQueryFromLine)(program.fix);
        }
    ...
    if (!programFile) {
        try {
            queries = (0, reader_1.getQueryFromLine)(fs.readFileSync(0).toString());
            prefix = "stdin";
        }
        catch (error) {
    ...
    if (program.host || (configuration === null || configuration === void 0 ? void 0 : configuration.host)) {
        db = (0, databaseFactory_1.default)(driver, program.host || (configuration === null || configuration === void 0 ? void 0 : configuration.host) || "localhost", program.user || (configuration === null || configuration === void 0 ? void 0 : configuration.user) || "root", // bad practice but unfortunately common, make it easier for the user
        ...
        program.database || (configuration === null || configuration === void 0 ? void 0 : configuration.database) || undefined);
    }
    if (programFile) {
        if (fs.lstatSync(programFile).isDirectory()) {
            const sqlFiles = (0, file_1.findByExtension)(programFile, "sql");
            for (const sqlFile of sqlFiles) {
                queries = (0, reader_1.getQueryFromFile)(sqlFile);
                await runner.run(queries, printer, sqlFile, omittedErrors, driver, db);
            }
        }
        else {
            queries = (0, reader_1.getQueryFromFile)(programFile);
            prefix = programFile;
        }
    }
