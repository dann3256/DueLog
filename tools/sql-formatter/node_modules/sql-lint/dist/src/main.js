...
exports.default = async ({ sql, host, port, database, user = "", prefix = "", password = "", verbosity = 0, driver = "mysql", }) => {
    const printer = new printer_1.Printer(verbosity, new json_1.JsonFormat());
    let db;
    if (host) {
        db = (0, databaseFactory_1.default)(driver, host, user, password, port, database);
    }
    const runner = new checkerRunner_1.CheckerRunner();
    await runner.run((0, reader_1.putContentIntoLines)(sql), printer, prefix, [], driver, db);
    if (db) {
        db.end();
    }
